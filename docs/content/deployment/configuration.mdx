...
| `notification.otp.max_generation_attempts` | `3` | Maximum number of OTP generation attempts per session. When a client requests a new OTP without providing the previous session token, the attempt count resets to 1. When the previous session token is provided, the server validates the recipient binding and increments the attempt count. If the limit is exceeded, OTP generation fails with an error. Valid range: `[1, 10]`. |
