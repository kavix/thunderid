...
2. **Deployment Configuration** — Located at `repository/conf/deployment.yaml` in your <ProductName /> installation directory. This file overrides specific defaults for your deployment.
...
<ProductName /> ships with a self-signed certificate for local development at `repository/resources/security/server.cert`. For production, replace it with a certificate from a trusted Certificate Authority.
:::
  ...
  configMap:
    ...
    items:
      ...
      - key: org-default
        ...
        mountPath: /opt/{{productSlug}}/repository/resources/organization_units/default.yaml
```
